...
export class StayService {
  ...
  public toggleLike(stayId: number, userId: number) {
    return this.httpService.post(this.STAY_URL + `${stayId}/like`, {
      userId,
    }) as Observable<Stay>;
  }
  ...
  public async loadStays(index: number = 0, isConnectStays: boolean = true) {
    ...
    const stays = (await lastValueFrom(
      this.httpService.get(this.STAY_URL + queryParams, null)
    )) as Stay[];
  ...
  public async loadFullLength() {
    ...
    const stayLength = (await lastValueFrom(
      this.httpService.get(this.STAY_URL + 'length/' + queryParams, null)
    )) as number;
  ...
  public save(stay: any) {
    if (stay.id)
      return lastValueFrom(this.httpService.put(this.STAY_URL, stay));
    return lastValueFrom(this.httpService.post(this.STAY_URL, stay));
  }

  public async addReview(stayId: number, review: Review): Promise<Review> {
    const savedReview = (await lastValueFrom(
      this.httpService.post(`${this.STAY_URL}${stayId}/reviews`, review)
    )) as Review;
  ...
  private getQueryParams(filterBy: StayFilter, index: number = 0) {
    ...
    if (filterBy.isPetAllowed)
      params += `isPetAllowed=${filterBy.isPetAllowed}&`;
