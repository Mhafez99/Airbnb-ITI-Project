...
export class EditStayComponent {
  ...
  async loadStay(stayId: string) {
    try {
      this.stay = (await lastValueFrom(
        this.stayService.getById(stayId)
      )) as any;
      this.loadImg();
    } catch (err) {
      console.log(err);
  ...
  async onAddStay() {
    ...
    this.stay.loc.address = `${city}, ${country}`;
    try {
      await this.stayService.save(this.stay);
      this.snackBar.open('Stay saved successfully', 'Close', {
        duration: 3000,
      });
  ...
  checkValidation() {
    ...
    if (stay.imgUrls.length < 2) {
      this.snackBar.open('You must add 2 images', 'Close', { duration: 3000 });
      return false;
    }
    if (stay.capacity < 1) {
      this.snackBar.open('You must add at least 1 capacity', 'Close', {
        duration: 3000,
      });
      return false;
    }
    ...
    if (stay.price < 1) {
      this.snackBar.open('You must add valid price', 'Close', {
        duration: 3000,
      });
      return false;
    }
    if (!/^[a-zA-Z]{3,}$/.test(stay.loc.country)) {
      this.snackBar.open('You must add valid country', 'Close', {
        duration: 3000,
      });
      return false;
    }
    if (!/^[a-zA-Z]{2,}$/.test(stay.loc.city)) {
      this.snackBar.open('You must add valid city', 'Close', {
        duration: 3000,
      });
      return false;
    }
    if (!/^[a-zA-Z][a-zA-Z0-9\s]{2,}$/.test(stay.loc.address)) {
      this.snackBar.open('You must add valid address', 'Close', {
        duration: 3000,
      });
      return false;
    }
